...
//+kubebuilder:validation:XValidation:rule="self.spec.source.type == 'git' ? has(self.spec.source.git) : true",message="git field is required when source type is 'git'"
//+kubebuilder:validation:XValidation:rule="self.spec.source.type == 'git' ? [has(self.spec.source.git.branch), has(self.spec.source.git.tag), has(self.spec.source.git.commit)].filter(x, x).size() <= 1 : true",message="only one of branch, tag, or commit may be specified for git source"
